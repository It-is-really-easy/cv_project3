...
@jit
def calculate_magnitude_and_direction(dx, dy, boundary_x, boundary_y, height, width, numChannels):
...
@jit
def build_feature_map(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize):
...
@jit
def normalize_and_truncate(partOfNorm, mappmap, sizeX, sizeY, p, xp, pp):
...
@jit
def pca_feature_map(mappmap, p, sizeX, sizeY, pp, yp, xp, nx, ny):
...
def getFeatureMaps(image, k, mapp):
    ...
    # 200x speedup
    r, alfa = calculate_magnitude_and_direction(dx, dy, boundary_x, boundary_y, height, width, numChannels)  # with @jit
    # ~0.001s
    ...
    # 500x speedup
    mapp['map'] = build_feature_map(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize)  # with @jit
    # ~0.001s
...
def normalizeAndTruncate(mapp, alfa):
    ...
    # 30x speedup
    newData = normalize_and_truncate(partOfNorm, mapp['map'], sizeX, sizeY, p, xp, pp)  # with @jit
    ###
...
def PCAFeatureMaps(mapp):
    ...
    # 190x speedup
    newData = pca_feature_map(mapp['map'], p, sizeX, sizeY, pp, yp, xp, nx, ny)  # with @jit
    ###
